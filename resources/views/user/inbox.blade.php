    ...
    <div class="container">
      ...
      <div class="mt-4 mb-2">
        <div class="my-account">
          <p>WELCOME TO ACCOUNT MESSAGE. HERE YOU CAN FIND ALL THE QUERY NOTIFICATION INFORMATION AND MESSAGE.</p>
        </div>
      </div>
                  ...
                  <a href="javascript:void(0)" id="{{$data->id}}">
                    <div class="chat_list">
                      <div class="chat_people">
                        ...
                        @if ($data->type == 0)
                          <div class="chat_ib">
                            <h5>Query - #0{{$data->query_id.$data->user_id}}
                              <span class="chat_date">{{$data->created_at->format('M d, Y ')}}<br>
                                @if ($data->is_open == 1)
                        ...
                        @elseif ($data->type == 1)
                          <div class="chat_ib">
                            <h5>Invoice - #90872<span class="chat_date">{{$data->created_at->format('M d, Y ')}}</span></h5>
                          </div>
                        @elseif ($data->type == 2)
                          <div class="chat_ib">
                            <h5>Booking Confirm <span class="chat_date">{{$data->created_at->format('M d, Y ')}}</span></h5>
                          </div>
                        @endif
                      </div>
                    </div>
